...
// udaje o jednom policku plochy: suradnice, hodnota (0 ak prazdne, alebo 1..9), a zoznam skupin, do ktorych patri.
struct policko {
int riadok, stlpec;
int hodnota;
int skupiny[SKUPIN];
...
//Mozeme ulozit danu hodnotu na policko s poradovym cislom pozicia? Da sa to, ak ziadna zo skupin tohto policka nema tuto hodnotu uz pouzitu.
...
//na policko s poradovym cislom pozicia uloz danu hodnotu. Ak je hodnota > 0, zaregistruj ju tiez ako obsadenu vo vsetkych skupinach, kam policko patri.
...
//hodnotu v policku s pozicia zmen na nulu a povodnu hodnotu odznac v poliach obsadene pre vsetky skupiny tohto policka
...
//inicializuj strukturu sudoku na zaklade vstupnej matice s cislami 0..9
s.pocetPolicok = 9 * 9;
s.pocetSkupin = 3 * 9;
...
//pre kazde policko naplnime jeho strukturu a vyplnime obsadene cifry
int pozicia = 0;
for (int i = 0; i < 9; i++) {
for (int j = 0; j < 9; j++) {
